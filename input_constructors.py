...
@acqf_input_constructor(MaxValueSelfCorrecting)
def construct_inputs_ScoreBO_M(
    model: Model,
    training_data: MaybeDict[SupervisedDataset],
    bounds: List[Tuple[float, float]],
    posterior_transform: Optional[PosteriorTransform] = None,
    ...
    **kwargs: Any,
    ...
    try:
        ...
        print('Matheron paths not available, falling back to discrete Thompson sampling.')
        from botorch.utils.sampling import sample_optimum_thompson

        from botorch.acquisition.max_value_entropy_search import _sample_max_value_Gumbel
        sobol = SobolEngine(dimension=training_data.X().shape[1], scramble=True)
        candidate_set = sobol.draw(num_thompson)
        optimal_inputs, optimal_outputs = _sample_max_value_Gumbel(
            model,
            candidate_set,
            num_optima
    ...
    optimal_inputs, optimal_outputs = optimal_inputs.to(
        dtype), optimal_outputs.to(dtype)
    #from botorch.utils.plot_acq import plot_optimal, plot_optimal_paper
    ##plot_optimal(paths, optimal_inputs, optimal_outputs, model)
    #plot_optimal_paper(paths, optimal_inputs, optimal_outputs, model)
...
@acqf_input_constructor(JointSelfCorrecting)
def construct_inputs_ScoreBO_J(
    model: Model,
    training_data: MaybeDict[SupervisedDataset],
    bounds: Tensor,
    num_optima: int = 8,
    maximize: bool = True,
    sample_around_best_std: int = 0.1,
    ...
    **kwargs: Any
    ...
    #    # plot run args:
    #    model.likelihood.noise = torch.Tensor([[0.03], [0.7], [0.02]])
    #    model.mean_module.constant = torch.Tensor([[0.01], [0.5], [-0.2]])
    #    model.covar_module.outputscale = torch.Tensor([[2], [0.1], [0.8], ])
    #    model.covar_module.base_kernel.lengthscale = torch.Tensor(
    #        [[[0.16]], [[0.4]], [[0.06]]])
    #
    #    model.train_inputs[0][:, 0] = 0.1
    #    model.train_inputs[0][:, 1] = 0.5
    #    model.train_inputs[0][:, 2] = 0.63
    #    model.train_inputs[0][:, 3] = 0.79
    #
    #    model.train_targets[:, 0] = 0.0
    #    model.train_targets[:, 1] = 1.6
    #    model.train_targets[:, 2] = 2.2
    #    model.train_targets[:, 3] = -1
    #model.train_inputs[0][:, 0] = 0.63
...
@acqf_input_constructor(StatisticalDistance)
def construct_inputs_SAL(
    model: Model,
    training_data: MaybeDict[SupervisedDataset],
    bounds: Tensor,
    noisy_distance: bool = False,
    **kwargs: Any
):
    inputs = {
        'model': model,
...
@acqf_input_constructor(qAdaptiveStatisticalDistance)
def construct_inputs_SAL(
    model: Model,
    training_data: MaybeDict[SupervisedDataset],
    bounds: Tensor,
...
@acqf_input_constructor(QBMGP)
def construct_inputs_QBMGP(
    model: Model,
    training_data: MaybeDict[SupervisedDataset],
    bounds: Tensor,
    noisy_distance: bool = False,
...
@acqf_input_constructor(VariationalEntropySearch) 
def construct_inputs_qVES(
    model: Model,
    training_data: MaybeDict[SupervisedDataset],
    bounds: Tensor,
    maximize: bool = True,
    ...
    X_pending: Optional[Tensor] = None,
    path_dtype: torch.dtype = 'float',
    num_optima: float = 128,
    plot: bool = False,
    num_features: int = 2 ** 11,
    **kwargs: Any
):
    if path_dtype == 'float':
        path_dtype = torch.float32
