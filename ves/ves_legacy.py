...
def plot_ves(ves, train_X=None, train_Y=None, f=None):
    ...
    axes[0].plot(dt(X).flatten(), m.flatten(), linewidth=2, label='posterior mean', color='tab:blue')
    ...
    axes[0].fill_between(dt(X).flatten(), (m - 2 * s).flatten(), (m + 2 * s).flatten(), alpha=0.2, color='tab:blue')
    axes[0].legend()

    kvals, betavals, ei, val = ves(X, return_ves=True)
    axes[1].plot(X.flatten(), dt(kvals.flatten()), linewidth=2, label=f'k', color='navy')
    axes[1].plot(X.flatten(), dt(betavals.flatten()), linewidth=2, label=f'beta', color='green')
    val = dt(val.flatten())
...
class VariationalEntropySearch(MCAcquisitionFunction):
    ...
    def forward(self, X, beta: float = 1.0, k: float = 1.0, return_ves = False):
        ...
        if return_ves:
            return kvals, betavals, improvement_term.mean(dim=0).squeeze(-1), ((reg_term_new + (kvals - 1) * torch.log(max_value_term) - betavals * max_value_term)).mean(dim=0).squeeze(-1).squeeze(-1)
...
def run_2d_experiment(f: Callable,
                      bounds: Tensor,
                      num_init: int,
...
def plot_ves_2d(model: VariationalEntropySearch,
                f: Callable,
                bounds: Tensor,
                nx: Optional[int] = 101,
                ny: Optional[int] = 101,
